...
def run_report(budget_file: BudgetFile, payload: dict):
    ...
    if report_type == SavedReport.TYPE_NET_WORTH:
        as_of = (
            _report_date(payload.get("as_of"), "as_of")
...
def _validate_template_posting_targets(budget_file, postings: list[dict]):
    ...
    """
    account_ids = {
        p.get("account_id") for p in postings if p.get("account_id") is not None
    }
    category_ids = {
        p.get("category_id") for p in postings if p.get("category_id") is not None
    }
    ...
    if account_ids:
        ...
        if unknown:
            raise ValueError(
                f"Unknown account ids for this budget file: {sorted(unknown)}"
            )
    ...
    if category_ids:
        ...
        if unknown:
            raise ValueError(
                f"Unknown category ids for this budget file: {sorted(unknown)}"
            )
